...
need to ignoreselfsigned.  You do this by not setting "-i"`);
        } else if (error.code && error.code === 'UNABLE_TO_GET_ISSUER_CERT_LOCALLY') {
            console.error(`${chalk.red.bold('ERROR:')} Your proxy is probably MITMing your HTTPS connections.
You need to trust the CA issed by your proxy.  Try using "--cafile" supply the path to your proxy's
...
parser.addArgument(['-P', '--password'], {
    help: 'The password to your proxy server. Please clear your shell history if you use this.'
});
parser.addArgument(['-c', '--cafile'], {
    help: `A path to a file containing one or multiple Certificate Authority signing
...
parser.addArgument(['-s', '--capassphrase'], {
    help: `The secret for your encrypted CA file.
...
console.log(msg);
console.log(`${chalk.green('********************************************************************************')}\n`);
probeURI(args['dest'], args['proxy'], args['ignoreselfsigned'], args['user'], args['password'], args['cafile']);
